...
func main() {
	...
	// routing
	namedTypedProductHandler := handler.AppHandler(h.HandleProducts)

	// Combine logging and auth middlewares
	loggedHandler := middleware.LoggingMiddleware(namedTypedProductHandler)
	protectedHandler := authMiddleware(loggedHandler)

	http.Handle("/products", protectedHandler)
